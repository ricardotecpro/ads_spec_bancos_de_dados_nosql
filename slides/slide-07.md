...
- `SUM()`: Somar 💰
- `AVG()`: Média 📈
- `MIN()` e `MAX()`: Extremos 📉 <!-- .element: class="fragment" -->
...
- Use `COUNT(DISTINCT cidade)` para valores únicos. <!-- .element: class="fragment" -->
...
- O `AS` deixa o resultado bonitinho! <!-- .element: class="fragment" -->
...
GROUP BY disciplina;
```
- Divide a tabela em "baldes" antes de calcular. <!-- .element: class="fragment" -->
...
HAVING AVG(nota) > 7;
```
- `HAVING` é o WHERE dos grupos! 🎯 <!-- .element: class="fragment" -->
...
```
- Profissionalismo e clareza nos relatórios. 🤝 <!-- .element: class="fragment" -->
...
6. `ORDER BY`: Qual a ordem? <!-- .element: class="fragment" -->
...
- Qual categoria é a campeã da sua loja? 🏆 <!-- .element: class="fragment" -->
