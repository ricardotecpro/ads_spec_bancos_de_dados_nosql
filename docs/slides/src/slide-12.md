...
- Dica: Comece pelo Compass para ver os dados! { .fragment }
...
- Use `.count()` para saber quantos tem. { .fragment }
...
- `$lt`: Menor que (< )
- `$in`: Está na lista 🧺
- `$or`: Uma coisa ou outra 🔀 { .fragment }
...
```
- **Atenção**: Sem o `$set`, você substitui o documento inteiro! 🛑 { .fragment }
...
```
- Rápido e direto.
- `deleteMany({})` limpa tudo! (Use com cautela). 🧨 { .fragment }
